...
const ContactScreen = ({navigation}: Props) => {
  ...
  const fetchContact = useCallback(async () => {
    const res = await axios.get((process.env.DEV_URL as string) + '/contact');
    ...
    <View className="bg-main min-h-full px-3 py-4">
      <View className="bg-white rounded-xl px-5 py-4">
        ...
        <TouchableOpacity
          ...
          <View className="ml-2">
            <Text className="text-lg font-sourceSansProSemiBold text-primary">Email</Text>
            <Text className="text-base font-sourceSansPro text-primary -mt-1">{contacts?.email}</Text>
          </View>
        </TouchableOpacity>
      </View>
      <View className="bg-white rounded-xl px-5 py-4 mt-4">
        ...
        <Each render={({platform, link,username, icon}) => (
          <TouchableOpacity
            ...
            <View className="ml-2">
              <Text className="text-lg font-sourceSansProSemiBold text-primary">{platform}</Text>
              <Text className="text-base font-sourceSansPro text-primary -mt-1">{username}</Text>
            </View>
          </TouchableOpacity>
        )} of={contacts?.socmed || null} ifNull={<></>} />
      </View>
    </View>
  );
};
